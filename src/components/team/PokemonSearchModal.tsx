...
export function PokemonSearchModal({ open, onClose, onSelect, versionGroupId }: PokemonSearchModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-start justify-center pt-[10vh]">
      ...
      {/* Modal */}
      <div className="relative w-full max-w-lg mx-4 rounded-lg border border-border bg-card shadow-xl flex flex-col max-h-[70vh]">
        {/* Header */}
                  ...
                  >
                    <div className="relative w-10 h-10 shrink-0">
                      {poke.spriteDefault ? (
                        ...
                        <div className="w-full h-full rounded bg-secondary flex items-center justify-center text-xs text-muted-foreground">?</div>
                      )}
                    </div>
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2">
                        <span className="text-sm font-medium truncate">{poke.name}</span>
